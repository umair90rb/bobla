...
class _DrawerWidgetState extends StateMVC<DrawerWidget> {
  ...
  getFirebaseId() async {
    print("User Email Id in drawer"+currentUser.value.email);
    FirebaseFirestore.instance
        .collection('users')
        .where("email", isEqualTo: currentUser.value.email).limit(1)
        .snapshots()
        .listen((data) { print("user id from firebase with listen "+data.docs[0].id);  return data.docs[0].id; });
  }
...
//              S.of(context).brands,
          ...
          ListTile(
            onTap: () async  {
              if(currentUser.value.email == "" || currentUser.value.email == null) {
              ...
              Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (context) =>
                          ChatRoomScreen(currentUserId:doc[0].reference.id)));
            }
              },
            leading: Icon(
              UiIcons.chat,
